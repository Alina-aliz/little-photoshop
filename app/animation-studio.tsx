...
export const AnimationStudio = forwardRef<AnimationStudioHandle, Props>(function AnimationStudio({ active, documentName, onModeChange, exportProject, getIllustrationImage, illustrations, photoLibrary, importSharedPhoto }, ref) {
  ...
  return <main className={`animator-shell ${active ? '' : 'mode-hidden'}`} aria-hidden={!active}>
    <header className="topbar animator-topbar"><div className="animator-brand"><div className="brand-mark"><Sparkles size={15} /></div><strong>DUET</strong><span>ANIMATOR</span></div><div className="mode-switch" aria-label="Workspace mode"><button onClick={() => { setPlaying(false); onModeChange('illustration'); }}>Illustrate</button><button className="active">Animate</button></div><div className="header-actions"><Button variant="ghost" size="sm" onClick={saveFrame}><Download />Save frame</Button><Button variant="ghost" size="sm" onClick={() => void exportAnimation()} disabled={exporting}><Download />{exporting ? 'Rendering…' : 'Export WebM'}</Button><Button size="sm" className="export-button" onClick={() => void exportWorkspace()} disabled={exporting} title="Download the animation and editable DUET project"><Download />Export workspace</Button></div></header>
