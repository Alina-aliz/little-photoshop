...
export const AnimationStudio = forwardRef<AnimationStudioHandle, Props>(function AnimationStudio({ active, documentName, onModeChange, exportProject, getIllustrationImage, illustrations, photoLibrary, importSharedPhoto }, ref) {
  ...
  const addFrame = (duplicate = false) => { if (activeClip?.type !== 'cel') return; const id = uid('frame'); const canvas = makeCanvas(); const source = frameCanvases.current.get(activeFrameId); if (duplicate && source) canvas.getContext('2d')!.drawImage(source, 0, 0); frameCanvases.current.set(id, canvas); const index = activeFrameIndex + 1; setClips((items) => items.map((clip) => { if (clip.id !== activeClip.id || clip.type !== 'cel') return clip; const frameIds = [...clip.frameIds.slice(0, index), id, ...clip.frameIds.slice(index)]; return { ...clip, frameIds, duration: celDuration(frameIds.length, clip.exposure, clip.finalHold) }; })); setActiveFrameId(id); setPlayhead(activeClip.start + index * activeClip.exposure); };
  ...
  const addCelClip = () => { if (!selectedTrack || selectedTrack.kind !== 'visual' || selectedTrack.locked) return; const frameId = uid('frame'); frameCanvases.current.set(frameId, makeCanvas()); const clip: CelClip = { id: uid('cel'), type: 'cel', trackId: selectedTrack.id, name: `Cel animation ${clips.filter((item) => item.type === 'cel').length + 1}`, start: playhead, duration: 2, opacity: 100, exposure: 2, finalHold: 0, frameIds: [frameId] }; setClips((items) => [...items, clip]); setActiveClipId(clip.id); setActiveFrameId(frameId); };
  const addStillCanvas = (canvas: HTMLCanvasElement, name: string) => { const track = selectedTrack?.kind === 'visual' && !selectedTrack.locked ? selectedTrack : tracks.find((item) => item.kind === 'visual' && !item.locked); if (!track) return; const clip: StillClip = { id: uid('still'), type: 'still', trackId: track.id, name: name.replace(/\.[^.]+$/, '').slice(0, 80) || 'Image', start: playhead, duration: fps * 3, opacity: 100 }; stillCanvases.current.set(clip.id, canvas); setClips((items) => [...items, clip]); setActiveTrackId(track.id); setActiveClipId(clip.id); setAssetMenuOpen(false); };
  ...
  const updateCelTiming = (patch: Partial<Pick<CelClip, 'exposure' | 'finalHold'>>) => setClips((items) => items.map((clip) => { if (clip.id !== activeClipId || clip.type !== 'cel') return clip; const exposure = patch.exposure ?? clip.exposure; const finalHold = patch.finalHold ?? clip.finalHold; return { ...clip, ...patch, exposure, finalHold, duration: celDuration(clip.frameIds.length, exposure, finalHold) }; }));
