...
export const AnimationStudio = forwardRef<AnimationStudioHandle, Props>(function AnimationStudio({ active, documentName, onModeChange, exportProject, getIllustrationImage, illustrations, photoLibrary, importSharedPhoto }, ref) {
  ...
  const addCelClip = () => { if (!selectedTrack || selectedTrack.kind !== 'visual' || selectedTrack.locked) return; const frameId = uid('frame'); const duration = 2; const start = findForwardSlot(clips, selectedTrack.id, playhead, duration); frameCanvases.current.set(frameId, makeCanvas()); const clip: CelClip = { id: uid('cel'), type: 'cel', trackId: selectedTrack.id, name: `Cel animation ${clips.filter((item) => item.type === 'cel').length + 1}`, start, duration, opacity: 100, exposure: 2, finalHold: 0, frameIds: [frameId] }; setClips((items) => [...items, clip]); setActiveClipId(clip.id); setActiveFrameId(frameId); setPlayhead(start); };
  const addStillCanvas = (canvas: HTMLCanvasElement, name: string) => { const track = selectedTrack?.kind === 'visual' && !selectedTrack.locked ? selectedTrack : tracks.find((item) => item.kind === 'visual' && !item.locked); if (!track) return; const duration = fps * 3; const start = findForwardSlot(clips, track.id, playhead, duration); const clip: StillClip = { id: uid('still'), type: 'still', trackId: track.id, name: name.replace(/\.[^.]+$/, '').slice(0, 80) || 'Image', start, duration, opacity: 100 }; stillCanvases.current.set(clip.id, canvas); setClips((items) => [...items, clip]); setActiveTrackId(track.id); setActiveClipId(clip.id); setPlayhead(start); setAssetMenuOpen(false); };
