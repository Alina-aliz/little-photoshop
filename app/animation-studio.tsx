...
import { Brush, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Copy, Download, Eraser, Eye, EyeOff, Film, Ghost, GripHorizontal, ImagePlus, Lock, Music2, Pause, Play, Plus, Redo2, Scissors, SkipBack, SkipForward, Sparkles, Trash2, Undo2, Unlock } from 'lucide-react';
...
const FRAME_EDITOR_DEFAULT_HEIGHT = 286;
const FRAME_EDITOR_MIN_HEIGHT = 136;
const FRAME_EDITOR_ONE_TRACK_HEIGHT = 190;
...
type Props = { active: boolean; documentName: string; onModeChange: (mode: 'illustration' | 'animation') => void; getIllustrationImage: () => ImageData | null };
...
function getMaxFrameEditorHeight() { return typeof window === 'undefined' ? FRAME_EDITOR_DEFAULT_HEIGHT : Math.max(FRAME_EDITOR_MIN_HEIGHT, Math.min(520, window.innerHeight - 290)); }
...
export function AnimationStudio({ active, documentName, onModeChange, getIllustrationImage }: Props) {
  ...
  const pointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => { if (playing || activeClip?.type !== 'cel' || activeTrack?.locked) return; event.currentTarget.setPointerCapture(event.pointerId); drawing.current = true; lastPoint.current = point(event); pushUndo(); drawLine(lastPoint.current, lastPoint.current); };
  const pointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => { if (!drawing.current) return; const next = point(event); drawLine(lastPoint.current, next); lastPoint.current = next; };
  const pointerUp = (event: React.PointerEvent<HTMLCanvasElement>) => { drawing.current = false; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); render(); };
  ...
  const importStill = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value = ''; if (!file?.type.startsWith('image/')) return; const url = URL.createObjectURL(file); const image = new Image(); image.onload = () => { const canvas = makeCanvas(); const scale = Math.min(WIDTH / image.width, HEIGHT / image.height); const width = image.width * scale; const height = image.height * scale; canvas.getContext('2d')!.drawImage(image, (WIDTH - width) / 2, (HEIGHT - height) / 2, width, height); URL.revokeObjectURL(url); const trackId = tracks.find((track) => track.kind === 'visual' && !track.locked)?.id; if (!trackId) return; const clip: StillClip = { id: uid('still'), type: 'still', trackId, name: file.name.replace(/\.[^.]+$/, ''), start: playhead, duration: fps * 3, opacity: 100 }; stillCanvases.current.set(clip.id, canvas); setClips((items) => [...items, clip]); setActiveClipId(clip.id); }; image.src = url; };
  ...
  return <main className={`animator-shell ${active ? '' : 'mode-hidden'}`} aria-hidden={!active}>
    <header className="topbar animator-topbar"><div className="animator-brand"><div className="brand-mark"><Sparkles size={15} /></div><strong>DUET</strong><span>ANIMATOR</span></div><div className="mode-switch" aria-label="Workspace mode"><button onClick={() => { setPlaying(false); onModeChange('illustration'); }}>Illustrate</button><button className="active">Animate</button></div><div className="header-actions"><Button variant="ghost" size="sm" onClick={saveFrame}><Download />Save frame</Button><Button size="sm" className="export-button" onClick={() => void exportAnimation()} disabled={exporting}><Download />{exporting ? 'Rendering…' : 'Export WebM'}</Button></div></header>
    <section className="animator-workspace"><aside className="animation-tool-rail" aria-label="Animation drawing tools"><button className={tool === 'brush' ? 'active' : ''} onClick={() => setTool('brush')} aria-label="Animation brush"><Brush /></button><button className={tool === 'eraser' ? 'active' : ''} onClick={() => setTool('eraser')} aria-label="Animation eraser"><Eraser /></button><span className="animation-rail-divider" /><label className="animation-color" title="Brush colour" aria-label="Brush colour"><input type="color" value={brushColor} onChange={(event) => setBrushColor(event.target.value)} /><span style={{ background: brushColor }} /></label></aside>
      <div className="animation-main hybrid" style={{ gridTemplateRows: `40px minmax(160px, 1fr) 42px ${frameEditorHeight}px` }}><div className="animation-context-bar"><strong>{tool === 'brush' ? 'Brush' : 'Eraser'}</strong><span>Size</span><Slider min={2} max={96} value={[brushSize]} onValueChange={(value) => setBrushSize(Math.round(Array.isArray(value) ? value[0] : Number(value)))} /><code>{brushSize}px</code>{tool === 'brush' && <><span>Opacity</span><Slider min={1} max={100} value={[brushOpacity]} onValueChange={(value) => setBrushOpacity(Math.round(Array.isArray(value) ? value[0] : Number(value)))} /><code>{brushOpacity}%</code></>}<span className="animation-context-spacer" /><Button variant="ghost" size="icon-sm" onClick={undo} aria-label="Undo frame stroke"><Undo2 /></Button><Button variant="ghost" size="icon-sm" onClick={redo} aria-label="Redo frame stroke"><Redo2 /></Button></div>
        <div className="animation-stage"><div className="animation-canvas-wrap"><canvas ref={displayRef} width={WIDTH} height={HEIGHT} className={`animation-canvas ${playing || activeClip?.type !== 'cel' ? 'playing' : ''}`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} /></div><div className="frame-counter">{Math.floor(playhead / fps)}:{String(playhead % fps).padStart(2, '0')} · frame {playhead + 1}</div></div>
        <div className="playback-bar"><button onClick={() => { setPlaying(false); setPlayhead(0); }} aria-label="Timeline start"><SkipBack /></button><button onClick={() => { setPlaying(false); setPlayhead((value) => Math.max(0, value - 1)); }} aria-label="Previous timeline frame"><ChevronLeft /></button><button className="play-button" onClick={() => setPlaying((value) => !value)} aria-label={playing ? 'Pause' : 'Play'}>{playing ? <Pause /> : <Play />}</button><button onClick={() => { setPlaying(false); setPlayhead((value) => Math.min(timelineFrames - 1, value + 1)); }} aria-label="Next timeline frame"><ChevronRight /></button><button onClick={() => { setPlaying(false); setPlayhead(timelineFrames - 1); }} aria-label="Timeline end"><SkipForward /></button><span className="playback-separator" /><label>FPS <input type="number" min={1} max={24} value={fps} onChange={(event) => setFps(clamp(Number(event.target.value) || 1, 1, 24))} /></label><button className={onionSkin ? 'active' : ''} onClick={() => setOnionSkin((value) => !value)}><Ghost />Onion skin</button></div>
        <div className={`hybrid-timeline ${timelineCollapsed ? 'timeline-collapsed' : ''}`}>
          <button type="button" className="frame-editor-resizer" aria-label="Resize frame editor. Drag or use the arrow keys." title="Drag to resize · Double-click to reset" onPointerDown={beginFrameEditorResize} onPointerMove={resizeFrameEditor} onPointerUp={endFrameEditorResize} onPointerCancel={endFrameEditorResize} onKeyDown={resizeFrameEditorWithKeyboard} onDoubleClick={() => setFrameEditorHeight(Math.min(FRAME_EDITOR_DEFAULT_HEIGHT, getMaxFrameEditorHeight()))}><GripHorizontal /></button>
