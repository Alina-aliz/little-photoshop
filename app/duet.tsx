...
export function Duet() {
  ...
  useEffect(() => {
    ...
    const tools: WebMCPTool[] = [
      { name: 'get_document_state', title: 'Inspect DUET document — WebMCP-only editing policy', description: 'Returns the active illustration surface, current selection, editable layers, and sent Agent edit bundle. This includes temporary cel layers while Illustrate cel is active in Animate. When agentEdit.ready is true, call prepare_ai_edit. Never use browser controls, clicks, drags, or local canvas drawing as a fallback.', execute: () => { const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? celState : actionsRef.current.getState()); } },
      ...
      { name: 'create_layer', title: 'Create an editable layer', description: 'Creates a transparent paint layer and makes it active on the current illustration surface, including Illustrate cel. This is not a fallback for a blocked AI edit.', inputSchema: { type: 'object', properties: { name: { type: 'string', description: 'Short layer name' } } }, execute: ({ name }) => { const layerName = typeof name === 'string' && name.trim() ? name : 'Agent layer'; const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? animationStudioRef.current?.createCelIllustrationLayer(layerName) : { layerId: actionsRef.current.createLayer(layerName) }); } },
      { name: 'set_active_tool', title: 'Choose an editing tool', description: 'Selects the transform, layer lasso, brush, eraser, smudge, blur, text, eyedropper, region selection, or pan tool on the active illustration surface. Do not use it or the browser UI to make a substitute manual edit when the requested AI edit is blocked.', inputSchema: { type: 'object', properties: { tool: { type: 'string', enum: ['select', 'layer-lasso', 'transform', 'brush', 'eraser', 'smudge', 'blur', 'text', 'eyedropper', 'pan'] } }, required: ['tool'] }, execute: ({ tool: next }) => { const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? animationStudioRef.current?.setCelIllustrationTool(String(next)) : { tool: actionsRef.current.setTool(next as Tool) }); } },
      { name: 'select_region', title: 'Select a canvas region', description: 'Creates the region that the next AI edit should modify on the active illustration surface. It does not authorize a browser-control or manual-canvas fallback.', inputSchema: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' }, width: { type: 'number' }, height: { type: 'number' } }, required: ['x', 'y', 'width', 'height'] }, execute: ({ x, y, width, height }) => { const selection = { x: Number(x), y: Number(y), width: Number(width), height: Number(height) }; const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? animationStudioRef.current?.selectCelIllustrationRegion(selection) : actionsRef.current.select(selection)); } },
      { name: 'prepare_ai_edit', title: 'Prepare the sent Agent edit bundle', description: 'Returns the frozen edit target crop, active-layer crop, mask, placement, and every ordered context reference from the active illustration surface, including temporary cel layers. Context images are reference-only. If the user has not sent a bundle, ask them to add selections and press Send to agent.', inputSchema: { type: 'object', properties: { prompt: { type: 'string', description: 'Optional visual edit instruction. Omit to let the agent decide.' } } }, execute: ({ prompt: edit }) => { const prompt = typeof edit === 'string' ? edit : ''; const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? animationStudioRef.current?.prepareCelIllustrationEdit(prompt) : actionsRef.current.prepareAiEdit(prompt)); } },
      { name: 'insert_ai_result', title: 'Insert an agent-generated image layer', description: 'Accepts a generated PNG, JPEG, or WebP data URL for a prepared bundle edit and creates a new editable layer on the originating illustration surface. In Illustrate cel it remains temporary until Flatten and return. Context references are never modified.', inputSchema: { type: 'object', properties: { editId: { type: 'string', description: 'Temporary ID returned by prepare_ai_edit' }, imageDataUrl: { type: 'string', description: 'Base64 image data URL for the complete prepared target crop' }, name: { type: 'string', description: 'Optional new layer name' } }, required: ['editId', 'imageDataUrl'] }, execute: async ({ editId, imageDataUrl, name }) => { const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? await animationStudioRef.current?.insertCelIllustrationResult(String(editId), String(imageDataUrl), typeof name === 'string' ? name : undefined) : await actionsRef.current.insertAiResult(String(editId), String(imageDataUrl), typeof name === 'string' ? name : undefined)); } },
      ...
      { name: 'merge_layer_down', title: 'Merge the selected layer down', description: 'Flattens the selected layer into the layer directly beneath it on the active illustration surface.', execute: () => { const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? animationStudioRef.current?.mergeCelIllustrationLayerDown() : actionsRef.current.mergeLayerDown()); } },
      { name: 'set_layer_visibility', title: 'Show or hide a layer', description: 'Changes layer visibility without destroying its pixels on the active illustration surface.', inputSchema: { type: 'object', properties: { layerId: { type: 'string' }, visible: { type: 'boolean' } }, required: ['layerId', 'visible'] }, execute: ({ layerId, visible }) => { const celState = animationStudioRef.current?.getCelIllustrationState(); return response(isRecord(celState) && celState.active ? animationStudioRef.current?.setCelIllustrationLayerVisibility(String(layerId), Boolean(visible)) : actionsRef.current.toggleLayer(String(layerId), Boolean(visible))); } },
    ];
