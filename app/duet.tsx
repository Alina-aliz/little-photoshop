...
export function Duet() {
  ...
  useEffect(() => {
    ...
    const tools: WebMCPTool[] = [
      ...
      { name: 'get_animation_state', title: 'Inspect DUET animation and AI clip request', description: 'Returns the animation canvas, playhead, tracks, supported coloured vector shapes, motion contract, and whether the user has pressed Send to agent. This first version sends no images. If aiClipRequest.ready is false, ask the user to press Send to agent in Animate and do not insert anything.', execute: () => response(animationStudioRef.current?.getAgentAnimationState() || { mode: 'animation', aiClipRequest: { ready: false, userInstruction: 'Open Animate and press Send to agent.' } }) },
      ...
      { name: 'insert_ai_cel_clip', title: 'Insert a coloured shape animation as editable cels', description: 'Validates a constrained coloured-shape and keyframe recipe, renders it into ordinary editable canvas cels, and inserts the cel clip on a new visual track. Requires the exact requestId returned by get_animation_state after the user presses Send to agent. Never submit scripts, SVG markup, URLs, or raster images.', inputSchema: { type: 'object', properties: {
        ...
        durationSeconds: { type: 'number', minimum: .25, maximum: 3, description: 'Short clip duration.' },
        celFps: { type: 'number', minimum: 1, maximum: 24, description: 'Desired cel rate. DUET maps it to the project timeline using cel holds.' },
      ...
      }, required: ['requestId', 'durationSeconds', 'celFps', 'objects'] }, execute: (input) => response(animationStudioRef.current?.insertAgentCelClip(input) || { inserted: false, error: 'Animation workspace is unavailable.' }) },
