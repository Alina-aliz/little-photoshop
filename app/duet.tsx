...
export function Duet() {
  ...
  useEffect(() => {
    ...
    const tools: WebMCPTool[] = [
      ...
      { name: 'get_animation_state', title: 'Inspect DUET animation and AI target request', description: 'Returns the animation canvas, tracks, selected cel-clip target range, adaptive sample manifest, supported coloured vector shapes, and whether the user pressed Send sampled frames. This call returns metadata only. If aiClipRequest.ready is false, ask the user to select a cel range and send it; do not insert anything.', execute: () => response(animationStudioRef.current?.getAgentAnimationState() || { mode: 'animation', aiClipRequest: { ready: false, userInstruction: 'Open Animate, select a cel range, and press Send sampled frames.' } }) },
